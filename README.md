...
**펫 모험 게임 (성장 + 활동):**
- 사용자별 고유 펫 캐릭터가 로컬 랜덤으로 생성됩니다 (추가 요금 없음).
- 성장(성공/실패)으로 활동 보상 배율이 변하고, 정산 시 골드를 회수할 수 있습니다.
- 활동: 산책(기본), 놀이(패스 드랍), 챌린지(패스 소모)로 골드를 획득합니다.
- 패스: 놀이 활동에서 일정 확률로 챌린지 패스를 얻어 고보상 미션을 시도합니다.
